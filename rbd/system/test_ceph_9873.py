...
def export_time():
    ...
    cmd(['rbd', '--cluster', '{}'.format(CLUSTER_NAME), 'export',
         '{}/clonetestimg'.format(POOL_NAME), '{}/clonetestimg'.format(DIR_PATH)])
...
if __name__ == "__main__":
    ...
    # Test Steps
    cmd(['rbd', '--cluster', '{}'.format(CLUSTER_NAME), 'create', '-s', '50G', '{}/testimg'.format(POOL_NAME)])
    cmd(['rbd', '--cluster', '{}'.format(CLUSTER_NAME), 'bench-write', '--io-total', '45G',
         '{}/testimg'.format(POOL_NAME)])
    cmd(['rbd', '--cluster', '{}'.format(CLUSTER_NAME), 'snap', 'create', '{}/testimg@snapimg'.format(POOL_NAME)])
    cmd(['rbd', '--cluster', '{}'.format(CLUSTER_NAME), 'snap', 'protect', '{}/testimg@snapimg'.format(POOL_NAME)])
    cmd(['rbd', '--cluster', '{}'.format(CLUSTER_NAME), 'clone', '{}/testimg@snapimg'.format(POOL_NAME),
    ...
    for ITERATION in range(1, 4):
        ...
        if ITERATION != 1:
            cmd(['rbd', '--cluster', '{}'.format(CLUSTER_NAME), 'feature', 'enable',
                 '{}/clonetestimg'.format(POOL_NAME), 'fast-diff'])
