# Script to test the RBD Positive CLI's (Import, Export and Diff CLIs)
#  Test Description:
#   a) Invoke various rbd cli commands(import, export, diff, import-diff, export-diff, merge-diff) with positive options
...
if __name__ == "__main__":
    ...
    cmd(['ceph', 'osd', '--cluster', '{}'.format(CLUSTER_NAME), 'pool', 'delete',
    ...
    # Export
    timer = datetime.datetime.now()
    for k, v in parameters['export_format_parameters'].iteritems():
        for ITERATOR in range(0, 2):
            cmd(['rbd', '--cluster', '{}'.format(CLUSTER_NAME), 'export', v['arg'], v['val'],
                 parameters['pool_name']['pool1']['val'] + '/' + 'testimg' + str(ITERATOR), '{}/'.format(PATH) +
                 'testimg' + str(ITERATOR)])

            cmd(['rbd', '--cluster', '{}'.format(CLUSTER_NAME), 'export', v['arg'], v['val'],
                 parameters['pool_name']['pool1']['val'] + '/' + 'testimg' + str(ITERATOR) + '@' + 'snapimg' + str(
                     ITERATOR), '{}/'.format(PATH) +
                        ...
                        for k5, v5 in parameters['image_shared_parameters'].iteritems():
                            ...
                            if k2 != k3:
                                ...
                                cmd(['rbd', '--cluster', '{}'.format(CLUSTER_NAME), 'import', v['arg'], v['val'],
                                     ...
                                     v4['arg'], v4['val'], v5['arg'], '{}/'.format(PATH) + 'testimg' + str(ITERATOR),
                                     parameters['pool_name']['pool1']['val'] + '/' + 'testimgimport' + str(ITERATOR3)])
                                ITERATOR3 += 1
    ...
    ITERATOR = 0
    ITERATOR2 = 0
    for k, v in parameters['whole_object_parameters'].iteritems():
        cmd(['rbd', '--cluster', '{}'.format(CLUSTER_NAME), 'export-diff', v['arg'],
             parameters['pool_name']['pool1']['val'] + '/' + 'testimg' + str(ITERATOR),
             '{}/'.format(PATH2) + 'testimg' + str(ITERATOR)])
        cmd(['rbd', '--cluster', '{}'.format(CLUSTER_NAME), 'export-diff', '--from-snap', 'snapimg' + str(ITERATOR),
             ...
             '{}/'.format(PATH3) + 'testimg' + str(ITERATOR)])
        cmd(['rbd', '--cluster', '{}'.format(CLUSTER_NAME), 'export-diff', v['arg'],
             parameters['pool_name']['pool1']['val'] + '/' + 'testimg' + str(ITERATOR) + '@' + 'snapimg' + str(
                 ITERATOR),
             '{}/'.format(PATH2) +
        ...
        if ITERATOR2 == 0:
            for path in [PATH2, PATH3]:
                ...
                cmd(['rm', '{}/'.format(path) + 'testimg' + str(ITERATOR) + '@' + 'snapimg' + str(ITERATOR)])
        ITERATOR2 += 1
    cmd(['rbd', '--cluster', '{}'.format(CLUSTER_NAME), 'export-diff',
         parameters['pool_name']['pool1']['val'] + '/' + 'testimg' + str(ITERATOR),
         '{}/'.format(PATH2) + 'testimgex' + str(ITERATOR)])
    print 'Execution time for Export-diff : ' + str(datetime.datetime.now() - timer)

    # Merge-diff
    timer = datetime.datetime.now()
    cmd(['rbd', '--cluster', '{}'.format(CLUSTER_NAME), 'merge-diff', '{}/'.format(PATH2) + 'testimg' + str(ITERATOR),
         '{}/'.format(PATH2) + 'testimgex' + str(ITERATOR), '{}/'.format(PATH) + 'merge-diff-img' + str(ITERATOR)])
    ...
    cmd(['ceph', 'osd', '--cluster', '{}'.format(CLUSTER_NAME), 'pool', 'delete',
         parameters['pool_name']['pool1']['val'],
         parameters['pool_name']['pool1']['val'], '--yes-i-really-really-mean-it'])
