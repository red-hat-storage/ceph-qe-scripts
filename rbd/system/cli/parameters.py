...
image_format = {"arg": "--image-format", "val": {None: None, "image-format 2": "2"}}

image_size = {"arg": "-s", "val": {"size_MB": "100M", "size_GB": "10G"}}

object_size = {
    "arg": "--object-size",
    "val": {None: None, "size_B": "8192B", "size_KB": "256K", "size_MB": "32M"},
}
...
io_size = {"arg": "--io-size", "val": {None: None, "size_KB": "256K"}}

io_threads = {"arg": "--io-threads", "val": {None: None, "val1": "20"}}

io_total = {"arg": "--io-total", "val": {"size_MB": "50M"}}

io_pattern = {
    "arg": "--io-pattern",
    "val": {None: None, "pattern_seq": "seq", "pattern_rand": "rand"},
}

limit = {"arg": "--limit", "val": {"val1": "10"}}

image_shared = {"arg": "", "val": {None: None, "val1": "--image-shared"}}

whole_object = {"arg": "", "val": {None: None, "val1": "--whole-object"}}


stripe_v2 = {
    "arg": ["--stripe-unit", "--stripe-count"],
    "val": {
        None: [None, None],
        "size_B": ["2048", "16"],
        "size_KB": ["65536", "16"],
...
io_type_v3 = io_type_v4 = {
    "arg": " --io-type",
    "val": {"read": "read", "write": "write"},
}
...
export_format_v3 = export_format_v4 = {
    "arg": "--export-format",
    "val": {None: None, "export-format 1": "1", "export-format 2": "2"},
}
...
class CliParams(object):
    def __init__(self, k_m=None, num_rep_pool=1, num_data_pool=0):
        ...
        for param in list:
            globals()[param] = globals()["{}_v{}".format(param, self.ceph_version)]
        for iterator in range(0, num_rep_pool):
            globals()["rep_pool"]["val"][
                "pool" + str(iterator)
            ] = self.rbd.random_string(length=5, prefix="rep_")
        ...
        if self.ceph_version > 2 and k_m:
            ...
            for iterator in range(0, num_data_pool):
                globals()["data_pool"]["val"][
                    "pool" + str(iterator)
                ] = self.rbd.random_string(length=5, prefix="data_")

            [self.rbd.create_pool(poolname=val) for key, val in rep_pool["val"].items()]
            [
                self.rbd.create_ecpool(poolname=val, profile=self.ec_profile)
                for key, val in data_pool["val"].items()
                if val != None
            ]
    ...
    def remove_duplicates(self, initial_list):
        final_list = []
        map(
            lambda val: final_list.append(val) if val not in final_list else False,
            initial_list,
        )
        return final_list
                ...
                if key:
                    if type(val) is list:
                        ...
                        for x in range(0, len(val)):
                            string = (
                                string + globals()[param]["arg"][x] + " " + val[x] + " "
                            )
