...
if __name__ == "__main__":
    ...
    combinations = cli.generate_combinations(
        "image_size", "object_size", "image_format"
    )
    if k_m:
        combinations = cli.generate_combinations(
            "image_size", "object_size", "image_format", "data_pool"
        )
        if cli.ceph_version == 3:
            invalid = [
                ...
                if (
                    cli.search_param_val("--image-format", val) != 0
                    and cli.search_param_val("--image-format", val).find("1") != -1
                )
            ]
    ...
    combinations = list(
        filter(
            lambda val: cli.search_param_val("-s", val).find("M") != -1
            and cli.search_param_val("--object-size", val) != 0
            and cli.search_param_val("--object-size", val).find("B") != -1,
            combinations,
        )
    )
    [
    ...
    [
        exec_cmd(
            "rbd snap create {}/img{num}@snapimg{num}".format(
                parameters.rep_pool["val"]["pool0"], num=iterator
            )
        )
    ...
    for iterator2 in range(0, iterator + 1):
        for iterator3, param in enumerate(combinations, start=0):
            exec_cmd(
                "rbd export {} {}/img{num} {}/img{num}".format(
                    param,
            ...
            exec_cmd(
                "rbd export {} {}/img{num}@snapimg{num} {}/img{num}@snapimg{num}".format(
                    param,
                    parameters.rep_pool["val"]["pool0"],
                    path_list[0],
                    num=iterator2,
            ...
            if cli.ceph_version > 2 and not (
                iterator3 == len(combinations) - 1 and iterator2 == iterator
            ):
                exec_cmd("rm {}/img{}".format(path_list[0], iterator2))
                exec_cmd(
                    "rm {}/img{num}@snapimg{num}".format(path_list[0], num=iterator2)
                )
    ...
    if cli.ceph_version == 2:
        invalid = [
            ...
            if (
                ...
                and cli.search_param_val("--image-feature", val).find("striping") != -1
    ...
    combinations = list(
        filter(
            lambda val: (
                cli.search_param_val("--image-format", val) == 0
                or cli.search_param_val("--image-format", val) != 0
                and cli.search_param_val("--image-format", val).find("1") == -1
            )
            and (
    ...
    [
        exec_cmd(
            "rbd import {} {}/img{} {} {}/imgimport{}".format(
                param,
                path_list[0],
                iterator,
    ...
    for iterator2, param in enumerate(combinations, start=0):
        exec_cmd(
            "rbd export-diff {} {}/img{num} {}/img{num}".format(
                param, parameters.rep_pool["val"]["pool0"], path_list[1], num=iterator
            )
        )
        exec_cmd(
            "rbd export-diff --from-snap snapimg{num} {} {}/img{num} {}/img{num}".format(
                param, parameters.rep_pool["val"]["pool0"], path_list[2], num=iterator
            )
        )
        exec_cmd(
            "rbd export-diff {} {}/img{num}@snapimg{num} {}/img{num}@snapimg{num}".format(
                param, parameters.rep_pool["val"]["pool0"], path_list[1], num=iterator
            )
        ),

        exec_cmd(
            "rbd export-diff --from-snap snapimg{num} {} {}/img{num}@snapimg{num} {}/img{num}@snapimg{num}".format(
                param, parameters.rep_pool["val"]["pool0"], path_list[2], num=iterator
            )
        )
        ...
        if iterator2 == 0:
            for path in path_list[1:]:
                ...
                exec_cmd("rm {}/img{num}@snapimg{num}".format(path, num=iterator))

    exec_cmd(
        "rbd export-diff {}/img{num} {}/imgex{num}".format(
            parameters.rep_pool["val"]["pool0"], path_list[1], num=iterator
        )
    )

    # Merge-diff
    exec_cmd(
        "rbd merge-diff {path}/img{num} {path}/imgex{num} {}/merge-diff-img{num}".format(
            path_list[0], path=path_list[1], num=iterator
        )
    )

    # Import-diff
    exec_cmd(
        "rbd import-diff {}/img{num} {}/img{num}".format(
            path_list[1], parameters.rep_pool["val"]["pool0"], num=iterator
        )
    )
    ...
    for param in combinations:
        ...
        exec_cmd(
            "rbd diff --from-snap snapimg{num} {} {}/img{num}".format(
                param, parameters.rep_pool["val"]["pool0"], num=iterator
            )
        )
        exec_cmd(
            "rbd diff {} {}/img{num}@snapimg{num}".format(
                param, parameters.rep_pool["val"]["pool0"], num=iterator
            )
        )
        exec_cmd(
            "rbd diff --from-snap snapimg{num} {} {}/img{num}@snapimg{num}".format(
                param, parameters.rep_pool["val"]["pool0"], num=iterator
            )
        )
