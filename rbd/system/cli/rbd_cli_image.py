...
if __name__ == "__main__":
    ...
    combinations = filter(
        lambda val: cli.search_param_val("-s", val).find("M") != -1, combinations
    )
    [
        exec_cmd(
    ...
    combinations = cli.generate_combinations(
        "image_size", "object_size", "stripe", "image_feature", "image_shared"
    )
    ...
    combinations = list(
        filter(
            lambda val: cli.search_param_val("--stripe-unit", val) == 0
            or (
    ...
    [
        exec_cmd(
            "rbd feature disable {}/img{} {}".format(
                parameters.rep_pool["val"]["pool0"], iterator + 2, val
            )
        )
        for val in image_feature
        if "layering" not in val and "striping" not in val
    ]

    for val in list(reversed(image_feature)):
        if (
            "deep-flatten" not in val
            and "layering" not in val
            and "striping" not in val
        ):
            exec_cmd(
                "rbd feature enable {}/img{} {}".format(
                    parameters.rep_pool["val"]["pool0"], iterator + 2, val
                )
            )
            if "fast-diff" in str(val) or "object-map" in str(val):
                exec_cmd(
                    "rbd object-map rebuild {}/img{}".format(
                        parameters.rep_pool["val"]["pool0"], iterator + 2
                    )
                )
    ...
    [
        exec_cmd(
            "rbd resize {} {}".format(
                param, parameters.rep_pool["val"]["pool0"] + "/img" + str(iterator + 2)
            )
        )
    ...
    [
        exec_cmd(
            "rbd mv {}/cpimg{index} {}/mvimg{index}".format(
                parameters.rep_pool["val"]["pool1"],
                parameters.rep_pool["val"]["pool1"],
                index=index,
            )
    ...
    index = 0
    exec_cmd(
        "rbd image-meta set {}/mvimg{} conf_rbd_cache false".format(
    ...
    # Image-meta list
    exec_cmd(
        "rbd image-meta list {}/mvimg{}".format(
            parameters.rep_pool["val"]["pool1"], index
        )
    )
    ...
    # Image-meta get
    exec_cmd(
        "rbd image-meta get {}/mvimg{} conf_rbd_cache".format(
            parameters.rep_pool["val"]["pool1"], index
        )
    )
    ...
    # Image-meta Removing
    exec_cmd(
        "rbd image-meta remove {}/mvimg{} conf_rbd_cache".format(
            parameters.rep_pool["val"]["pool1"], index
        )
    )
    ...
    # Image Info
    exec_cmd(
        "rbd info {}/img{}".format(parameters.rep_pool["val"]["pool0"], iterator + 2)
    )

    # Image Status
    exec_cmd(
        "rbd status {}/img{}".format(parameters.rep_pool["val"]["pool0"], iterator + 2)
    )
    ...
    if cli.ceph_version > 2:
        ...
        exec_cmd(
            "rbd trash mv {}/mvimg{}".format(parameters.rep_pool["val"]["pool1"], index)
        )
                ...
                if cli.ceph_version == 3:
                    exec_cmd(
                        "rbd trash restore {}/{}".format(
                            parameters.rep_pool["val"]["pool0"], json_output[num]
                        )
                    )
                else:
                    exec_cmd(
                        "rbd trash restore {}/{}".format(
                            parameters.rep_pool["val"]["pool0"], json_output[num]["id"]
                        )
                    )
        ...
        if exec_cmd("rbd trash ls {}".format(parameters.rep_pool["val"]["pool1"])):
            ...
            if cli.ceph_version == 3:
                exec_cmd(
                    "rbd trash remove {}/{}".format(
                        parameters.rep_pool["val"]["pool1"], json_output[0]
                    )
                )
            else:
                exec_cmd(
                    "rbd trash remove {}/{}".format(
                        parameters.rep_pool["val"]["pool1"], json_output[0]["id"]
                    )
                )
