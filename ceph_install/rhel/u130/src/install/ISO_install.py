...
class ISOInstall(object):
    ...
    def install_ceph(self):
        ...
        self.install_repo_in_mon_nodes = 'ceph-deploy install --repo --release=ceph-mon %s' %(' '.join(self.mon_hostnames))
        ...
        self.install_repo_in_osd_nodes = 'ceph-deploy install --repo --release=ceph-osd %s' %(' '.join(self.osd_hostnames))
