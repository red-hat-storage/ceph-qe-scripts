...
class CalamariToolsRepo(object):
    ...
    def set_repo(self):
        ...
        print 'setting repo'
        self.repo = "sudo bash -c 'umask 0077; echo deb https://%s:%s@download.inktank.com/enterprise-testing/Ceph-1.3-Ubuntu-14.04-20150813.t.0/repos/debian/Installer " \
    ...
    def enable_calamari_tools(self):
        ...
        calamari_repo_link = "'https://%s:%s@download.inktank.com/enterprise-testing/Ceph-1.3-Ubuntu-14.04-20150813.t.0/repos/debian/Calamari'" % (
        self.username, self.password)

        self.calamari_repo = "sudo ceph-deploy repo --repo-url  " + calamari_repo_link + " Calamari  `hostname -f`"

        tools_repo_link = "'https://%s:%s@download.inktank.com/enterprise-testing/Ceph-1.3-Ubuntu-14.04-20150813.t.0/repos/debian/Tools'" % (
        self.username, self.password)


        self.tools_repo = "sudo ceph-deploy repo --repo-url  " + tools_repo_link + " Tools `hostname -f`"
...
class MonRepos(object):
    ...
    def set_repo(self):
        ...
        repo_url = 'https://%s:%s@download.inktank.com/enterprise-testing/Ceph-1.3-Ubuntu-14.04-20150813.t.0/repos/debian/MON' % (self.username, self.password)
        ...
        self.repo = 'ceph-deploy repo --repo-url ' + repo_url + ' --gpg-url ' + gpg_url + ' Monitor ' + ' '.join(
            mon_hostname)
...
class OSDRepo(object):
    ...
    def set_repo(self):
        ...
        repo_url = 'https://%s:%s@download.inktank.com/enterprise-testing/Ceph-1.3-Ubuntu-14.04-20150813.t.0/repos/debian/OSD' % (self.username, self.password)
        gpg_url = 'https://download.inktank.com/keys/release.asc'
        ...
        self.repo = 'ceph-deploy repo --repo-url ' + repo_url + ' --gpg-url ' + gpg_url + ' OSD ' + ' '.join(
            osd_hostname)
...
class InstallRepo(object):
    ...
    def set_repos(self):
        self.calamari_repo = CalamariToolsRepo(self.username, self.password, self.admin_node)
...
class InstallCeph(object):
    ...
    def install_mon(self):
        ...
        self.install_mon_cmd = 'ceph-deploy install --no-adjust-repos --mon %s' %(' '.join(mon_hostname))
    ...
    def install_osd(self):
        ...
        self.install_osd_cmd = 'ceph-deploy install --no-adjust-repos --osd %s' %(' '.join(osd_hostname))
    ...
    def install_cli(self):
        ...
        self.install_cli_cmd = 'ceph-deploy install --no-adjust-repos --cli %s' %(self.admin_node.hostname)
...
class CDNInstall(object):
    ...
    def __init__(self, username, password, admin_node, mons, osds, set_repo):
        ...
        self.install_repo = InstallRepo(self.username, self.password, self.admin_node, self.mons, self.osds)
