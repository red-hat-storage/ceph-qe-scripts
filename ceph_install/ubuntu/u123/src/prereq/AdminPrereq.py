...
class AdminFireWallSettings(object):
    ...
    def firewall_settings_commands(self):
        ...
        self.open_80 = (
            " ' sudo iptables -I INPUT 1 -i eh0 -p tcp -s %s --dport 80 -j ACCEPT' "
            % (self.host.ip)
        )
        self.open_2003 = (
            " 'sudo iptables -I INPUT 1 -i eth0 -p tcp -s %s --dport 2003 -j ACCEPT' "
            % (self.host.ip)
        )
        self.open_45005_4506 = (
            " 'sudo iptables -I INPUT 1 -i eth0 -m multiport -p tcp -s %s --dports 4505:4506 -j ACCEPT' "
            % (self.host.ip)
        )
