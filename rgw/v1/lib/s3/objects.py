...
from v1.lib.s3.json_ops import JMulpipart
...
class PutContentsFromString(object):
    ...
    def check_contents(self):
        ...
        try:
            ...
            string_exists_status = {"status": True, "contents": string_contents}
        except exception.BotoClientError as e:
            log.error(e)
            string_exists_status = {"status": False, "msgs": e}
...
class PutContentsFromFile(object):
    ...
    def put(self, filename, test_op_code="create"):
        ...
        try:
            ...
            self.add_io_info.add_keys_info(
                self.key.bucket.connection.access_key,
    ...
    def get(self, filename):
        ...
        except (exception.BotoClientError, exception.S3ResponseError, Exception) as e:
            log.error(e)
            ...
            if not os.path.exists(json_file):
                ...
                for each_file in self.split_files_list:
                    remaining_file_parts.append(
                        (each_file, (self.split_files_list.index(each_file) + 1))
                    )
    ...
    def put(self):
        try:
            ...
            if self.break_at_part_no == 0:
                ...
                self.add_io_info.add_keys_info(
                    self.bucket.connection.access_key,
                    self.bucket.name,
                    **{
                        "key_name": os.path.basename(self.filename),
