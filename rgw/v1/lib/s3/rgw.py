...
from .objects import KeyOp, MultipartPut, PutContentsFromFile
    ...
    def set_bucket_properties(self):
        ...
        for bucket in self.buckets_created:
            ...
            if self.version_count is not None:
                self.bucket_ops.enable_disable_versioning(
                    bucket, self.enable_versioning
                )
        ...
        for bucket_created in buckets_created:
            ...
            for key in range(object_create_nos):
                ...
                if self.enable_versioning:
                    ...
                    keys_with_version = [
                        key_name + ".version." + str(i)
                        for i in range(self.version_count)
                    ]
                    log.info("version_key_names %s:\n" % keys_with_version)
                    files_with_version = [
                        utils.create_file(x, size) for x in keys_with_version
                    ]
                    ...
                    if self.move_version:
                        ...
                        log.info("listing all version")
                        version_details = [
                            {"key": k.name, "version_id": k.version_id}
                            for k in versions
                        ]
                        self.version_ids = [i["version_id"] for i in version_details]
                        list(map(log.info, version_details))
                    current_key_version_id = key_created.version_id
                    log.info(
                        "current_key_version_id after moving version: %s"
        ...
        for bucket in buckets_created:
            for object_count in range(self.objects_count):
                ...
                json_file = os.path.join(
                    os.path.dirname(filename), os.path.basename(filename) + ".json"
                )
    ...
    def delete_key_version(self):
        for bucket_name in self.bucket_names:
            ...
            for each_key in self.keys_put:
                ...
                list(map(del_key_version, self.version_ids))
