...
class Results:
    ...
    def add(self, tid, name, verdict, notes="", metrics=None):
        ...
        if verdict == "PASS":
            ok(f"[{tid}] {name}  {_c(CYAN, notes)}")
        elif verdict == "FAIL":
            fail(f"[{tid}] {name}  {notes}")
        elif verdict == "SKIP":
            skip(f"[{tid}] {name}  {notes}")
        else:
            info(f"[{tid}] {name}  {verdict} — {notes}")
...
def run_setup_checks(cfg, results):
    ...
    for node in clients:
        ...
        if device in out and "PORT_ACTIVE" in out:
            ...
            results.add(
                "SETUP-03",
                f"{node}: RDMA device {device} PORT_ACTIVE",
                "FAIL",
    ...
    if rc == 0 or out.strip() == "0":
        ...
        out2, _, _ = ssh(
            server,
            "ceph orch ps --daemon-type nfs 2>/dev/null | grep running | wc -l",
            cfg,
        )
        if out2.strip() and int(out2.strip()) > 0:
            results.add(
                "SETUP-04", f"{server}: nfs-ganesha daemon running (cephadm)", "PASS"
            )
        else:
            results.add(
                "SETUP-04",
                f"{server}: nfs-ganesha service active",
                "FAIL",
    ...
    for node in clients:
        ...
        if refcnt == "0" or refcnt == "":
            results.add(
                "SETUP-06", f"{node}: rpcrdma refcount clean (no stale handles)", "PASS"
            )
        else:
            results.add(
                "SETUP-06",
                f"{node}: rpcrdma refcount={refcnt}",
                "FAIL",
...
def mount_rdma(host, cfg, timeout=30):
    ...
    _, err, rc = ssh(
        host, f"mount -t nfs -o {opts} {vip}:{export} {mnt}", cfg, timeout=timeout
    )
    return rc == 0, err
...
def test_t01_rdma_hardware(host, cfg, results):
    ...
    out, _, _ = ssh(
        host, f"ibv_devinfo 2>/dev/null | grep -E 'hca_id|state|fw_ver'", cfg
    )
    ...
    results.add(
        ...
        "RDMA hardware validation",
        "PASS",
        f"device={device} fw={fw}",
        {"device": device, "fw_ver": fw},
    )
...
def test_t02_rdma_mount(host, cfg, results):
    ...
    if failed:
        results.add(
            ...
            "RDMA sync mount",
            "FAIL",
            f"Missing mount options: {failed}",
            {"mount_line": out},
        )
    else:
        results.add(
...
def test_t04_no_tcp_fallback(host, cfg, results):
    ...
    if rc != 0:
        results.add(
            "T-04",
            "No TCP fallback — wrong RDMA port fails cleanly",
            "PASS",
...
def test_t05_protocol_verify(host, cfg, results):
    ...
    if rc == 0 and delta > 0:
        results.add(
            "T-05",
            "Protocol verification — RDMA counter delta",
            "PASS",
            f"RX delta={delta}",
            {"rdma_rx_delta": delta},
        )
...
def test_t03_basic_file_ops(host, cfg, results):
    ...
    _, _, rc = ssh(
        host,
        f"dd if=/dev/urandom of={wdir}/t03.bin bs=1M count=10 2>/dev/null",
        cfg,
        timeout=60,
    )
    if rc != 0:
    ...
    out2, _, rc = ssh(
        host, f"md5sum {wdir}/t03_renamed.bin 2>/dev/null", cfg, timeout=30
    )
    ...
    if errors:
        results.add(
            "T-03",
            "Basic file ops (write/read/rename/delete)",
            "FAIL",
            ", ".join(errors),
        )
    else:
...
def test_t08_dual_mount(rdma_host, tcp_host, cfg, results):
    ...
    if not ok_tcp:
        results.add(
            "T-08",
            "Dual mount TCP+RDMA same export",
            "FAIL",
    ...
    _, _, rc1 = ssh(
        rdma_host,
        f"dd if=/dev/urandom of={wdir_rdma}/t08_rdma.bin bs=1M count=32 2>/dev/null",
        cfg,
        timeout=60,
    )
    ...
    out, _, rc2 = ssh(
        tcp_host,
        f"md5sum {tcp_mnt}/rdma-nfs-autotest/t08_rdma.bin 2>/dev/null",
        cfg,
        timeout=30,
    )
    ...
    _, _, rc3 = ssh(
        tcp_host,
        f"dd if=/dev/urandom of={tcp_mnt}/rdma-nfs-autotest/t08_tcp.bin bs=1M count=32 2>/dev/null",
        cfg,
        timeout=60,
    )
    ...
    out2, _, rc4 = ssh(
        rdma_host, f"md5sum {wdir_rdma}/t08_tcp.bin 2>/dev/null", cfg, timeout=30
    )
...
def test_t09_dir_depth(host, cfg, results, depth=20, files_per_level=1000):
    ...
    if total_written == expected:
        results.add(
            "T-09",
            f"Dir depth-{depth} + {files_per_level} files/level",
            "PASS",
...
def test_t25_file_locking(host, cfg, results):
    ...
    if "LOCKED" in out:
        results.add(
            "T-25",
            "File locking — concurrent writers blocked",
            "PASS",
...
def test_t11_mixed_sizes(host, cfg, results, sizes_kb):
    ...
    for kb in sizes_kb:
        ...
        ssh(
            host,
            f"dd if=/dev/urandom of={f} bs=1024 count={kb} 2>/dev/null",
            cfg,
            timeout=120,
        )
...
def test_t14_cpu_util(host, cfg, results, size_mb=1024):
    ...
    if rc == 0 and out:
        results.add(
            "T-14",
            "CPU utilisation during RDMA write",
            "PASS",
            f"mpstat captured ({size_mb}MB write)",
            {"mpstat_tail": out},
        )
    else:
        results.add(
...
def test_scale_flat_files(host, cfg, results, tid, name, total_files, file_size_bytes):
    ...
    if rc == 0 and actual >= total_files:
        ...
        results.add(
            tid, name, "FAIL", f"Only {actual}/{total_files} files created (rc={rc})"
        )
...
"""
    ...
    for d in range(1, depth + 1):
        ssh(
            host,
            f"python3 /tmp/t18_worker.py {d:02d} > /tmp/t18_d{d:02d}.log 2>&1 &",
            cfg,
            timeout=5,
        )
...
def test_t20_io_stability(host, cfg, results):
    ...
    if errors:
        results.add(
            "T-20", "I/O stability (sustained writes)", "FAIL", "; ".join(errors)
        )
    else:
        results.add(
            "T-20",
            "I/O stability (sustained writes)",
            "PASS",
...
def test_t21_ganesha_restart(server, client, cfg, results):
    ...
    if "rdma" not in out:
        results.add(
            "T-21",
            "Ganesha restart with active mount",
            "FAIL",
    ...
    _, _, rc1 = ssh(
        client,
        f"dd if=/dev/urandom of={wdir}/t21_pre.bin bs=1M count=10 2>/dev/null",
        cfg,
        timeout=30,
    )
    ...
    ssh(
        server,
        "ceph orch restart nfs.nfs-gateway 2>/dev/null || systemctl restart nfs-ganesha 2>/dev/null",
        cfg,
        timeout=30,
    )
    ...
    _, _, rc3 = ssh(
        client,
        f"dd if=/dev/urandom of={wdir}/t21_post.bin bs=1M count=10 2>/dev/null",
        cfg,
        timeout=30,
    )
...
def test_t22_rgw_restart(server, client, cfg, results):
    ...
    _, _, rc1 = ssh(
        client, f"dd if=/dev/urandom of={f} bs=1M count=64 2>/dev/null", cfg, timeout=60
    )
    ...
    _, _, rc3 = ssh(
        client,
        f"dd if=/dev/urandom of={f}.post bs=1M count=32 2>/dev/null",
        cfg,
        timeout=60,
    )
    ...
    if rc1 == 0 and rc2 == 0 and rc3 == 0:
        results.add(
            "T-22",
            "RGW restart with active RDMA connections",
            "PASS",
...
def test_t15_concurrent_rdma(
    clients, cfg, results, per_client_files, per_client_size_mb
):
    ...
    if not failed_clients:
        results.add(
            "T-15",
            f"{len(clients)} concurrent RDMA clients",
            "PASS",
...
def test_t16_mixed_concurrent(
    rdma_clients, tcp_clients_host, cfg, results, per_client_files, per_client_size_mb
):
    ...
    if not failed:
        results.add(
            "T-16",
            f"Mixed {len(rdma_clients)} RDMA + 1 TCP concurrent",
            "PASS",
...
def main():
    ...
    scale_cfg = (
        load_yaml(args.scale)
        if "scale" in categories and Path(args.scale).exists()
        else {}
    )
    perf_cfg = (
        load_yaml(args.perf)
        if "performance" in categories and Path(args.perf).exists()
        else {}
    )
    ...
    print(
        f"{_c(BOLD + CYAN,   '║   IBM Ceph NFS-RDMA Automated Test Suite             ║')}"
    )
    print(
        f"{_c(BOLD + CYAN,   '╚══════════════════════════════════════════════════════╝')}"
    )
    ...
    if "functional" in categories:
        ...
        if secondary:
            ...
            results.add(
                "T-08", "Dual mount TCP+RDMA", "SKIP", "No secondary client configured"
            )
    ...
    if "performance" in categories:
        ...
        for wt in p.get("write_tests", []):
            if wt.get("enabled", True):
                ...
                test_write_throughput(
                    primary, cfg, results, tid, wt["name"], wt["size_mb"]
                )
        ...
        for rt in p.get("read_tests", []):
            if rt.get("enabled", True):
                ...
                test_read_throughput(
                    ...
                    rt["name"],
                    rt["size_mb"],
                    rt.get("verify_md5", True),
                )

        # Mixed sizes
        ms = p.get("mixed_sizes", {})
        if ms.get("enabled", True):
            test_t11_mixed_sizes(
                primary, cfg, results, ms.get("sizes_kb", [4, 64, 1024, 10240, 102400])
            )
        ...
        if conc.get("mixed_enabled", False) and secondary:
            ...
            results.add(
                "T-16", "Mixed RDMA+TCP concurrent", "SKIP", "Need secondary client"
            )
