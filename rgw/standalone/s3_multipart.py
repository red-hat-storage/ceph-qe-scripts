...
for i in range(1, 51):
    ...
    upload_part1 = s3.upload_part(Body=open('testfile1', mode="rb"),
                                  Bucket='multipart', Key=k, PartNumber=1,
                                  UploadId=multi_part['UploadId'])
    upload_part2 = s3.upload_part(Body=open('testfile2', mode="rb"),
                                  Bucket='multipart', Key=k, PartNumber=2,
                                  UploadId=multi_part['UploadId'])
    response = s3.complete_multipart_upload(Bucket='multipart', Key=k,
