        ...
        for bc in range(config.bucket_count):
            ...
            if config.test_ops.get("verify_checksum_api", False):
                ...
                for oc, size in list(config.mapped_sizes.items()):
                    ...
                    if not complete_multipart_upload_resp["Checksum"]:
                    ...
                    if not attrib_resp["Checksum"]:
                        raise AssertionError(
                            "Checksum not available in object attributes"
                        )
                    elif list(attrib_resp["Checksum"].values())[0] != checksum:
                        raise TestExecError("Checksums do not match")
                ...
                for oc, size in list(config.mapped_sizes.items()):
                    ...
                    if "Checksum" not in list(attrib_resp.keys()):
                        raise AssertionError(
                            "Checksum not available in object attributes"
                        )
                    elif list(attrib_resp.values())[4] != checksum:
                        raise TestExecError("Checksums do not match")
