    ...
    for user in user_info:
        ...
        for bc in range(config.bucket_count):
            ...
            if config.test_ops.get("wrong_etag_first", True):
                ...
                log.info(f"AAA: wrong list is {wrong_list}")
            ...
            log.info("Conditional delete-objects with correct ETag for all %d objects", config.objects_count)
            log.info(f"BBBB {objects_with_etag}")
            ...
            log.info("All %d objects conditionally deleted successfully", config.objects_count)
