...
TEST_DATA_PATH = None
ACLS = {0: "private", 1: "public-read", 2: "public-read-write"}


def create_bucket(rgw_conn, user_info, rand_no=0):
    s3_ops = ResourceOps()
    bucket_name_to_create = utils.gen_bucket_name_from_userid(
        user_info["user_id"], rand_no
    )
    log.info("creating bucket with name: %s" % bucket_name_to_create)
    bucket = s3_ops.resource_op(rgw_conn, "Bucket", bucket_name_to_create)
    created = s3_ops.resource_op(
        bucket, "create", None, **{"access_key": user_info["access_key"]}
    )
    if created is False:
...
def test_acls_public_read(u1_rgw_conn, u1, u2_rgw_conn, u1_bucket, u2_bucket):
    ...
    object_uploaded_status = s3_ops.resource_op(
        u1_bucket, "upload_file", s3_object_path, s3_object_name, **upload_info
    )
