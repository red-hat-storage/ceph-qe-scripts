...
import traceback
from verify_on_s3 import ReadIOInfoOnS3
    ...
    try:
        ...
        if io_op_config.get('create', None) is True:
            ...
            for bc in range(io_config['basedir_count']):
                ...
                if io_config['file_count'] != 0:
                    ...
                    for fc in range(io_config['file_count']):
                        ...
                        file_size = utils.get_file_size(io_config['objects_size_range']['min'],
            ...
            if io_op_config.get('delete', None) is True:
                ...
                map(lambda x: shutil.rmtree(os.path.abspath(os.path.join(mnt_point, x))),
                    basedirs_list)
            ...
            if io_op_config.get('move', None) is True:
                ...
                for each_file in read_io_info_on_s3.objects:
                    if each_file['type'] == 'file':
                        ...
                        current_path = os.path.abspath(os.path.join(mnt_point, each_file['bucket'],
                                                                    each_file['name']))
                        new_path = os.path.abspath(os.path.join(mnt_point, each_file['bucket'],
                                                                    each_file['name'] + ".moved"))

                        moved = utils.exec_shell_cmd('sudo mv %s %s' % (current_path, new_path ))
