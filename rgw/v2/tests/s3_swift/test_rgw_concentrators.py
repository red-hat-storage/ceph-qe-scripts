...
def test_exec(config, ssh_con, rgw_node, test_info_obj):
    ...
    log.info(
        f"Detected Ceph version: {ceph_version_name} (ID: {original_ceph_version_id_for_log})"
    )
    if (
        ...
        log.info(
            f"Ceph version {ceph_version_name} meets the required version (>= 19.2.1). Proceeding with tests."
        )
    else:
        log.info(
        ...
        test_info_obj.success_status(
            f"Test skipped: Feature not valid for version less than 8.1 (detected: {ceph_version_name})"
        )
        return

    log.info("Creating io_info file for user information")
    io_info_file = (
        f"io_info_{os.path.basename(os.path.splitext(config.yaml_file)[0])}.yaml"
    )
    ...
    if not os.path.exists(io_info_path):
        try:
            ...
            os.chmod(
                io_info_path, stat.S_IRUSR | stat.S_IWUSR | stat.S_IRGRP | stat.S_IWGRP
            )
            log.info(
                f"Set permissions on {io_info_path}: {oct(os.stat(io_info_path).st_mode)[-3:]}"
            )
        ...
        log.info(
            f"Existing file permissions: {oct(os.stat(io_info_path).st_mode)[-3:]}"
        )
    ...
    if config.test_ops.get("perform_s3_operations", False):
        ...
        if not all_users_info:
            raise TestExecError(
                f"Failed to create {user_count} user(s) for S3 operations."
            )

        config.users = all_users_info
        if TEST_DATA_PATH is None:
            raise TestExecError(
                "TEST_DATA_PATH was not properly initialized in __main__."
            )

        if not concentrator_tests.perform_s3_operations(
            config, ssh_con, all_users_info, TEST_DATA_PATH
        ):
    ...
    if not (
        ...
        log.info(
            "Skipping RGW and HAProxy tests and S3 operations as per configuration"
        )
...
if __name__ == "__main__":
    test_info = AddTestInfo(
        "check RGW and HAProxy colocation and concentrator behavior and S3 operations"
    )
