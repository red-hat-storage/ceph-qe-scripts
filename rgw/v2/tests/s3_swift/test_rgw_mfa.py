...
def test_exec(config):
    ...
    # authenticate
        ...
        log.info ('Configure the one-time password generator oathtool and the back-end MFA system to use the same seed.')
        test_totp = reusable.generate_totp(SEED)
        serial = 'MFAtest' + str(random.randrange(1, 100))
        if test_totp is False:
             raise TestExecError ("Failed to configure one-time password generator - oathtool")
        ...
        if config.test_ops.get('mfa_resync_invalid_syntax') is True:
            log.info('Validate the mfa resync command errors out with approriate message on invalid syntax')
        ...
        if config.test_ops['mfa_check'] is True:
            log.info('Test a multi-factor authentication (MFA) time-based one time password (TOTP) token.')
            get_totp = reusable.generate_totp(SEED)
            cmd = 'time radosgw-admin mfa check --uid=%s --totp-serial=%s --totp-pin=%s'%(each_user['user_id'],serial,get_totp)
            ...
            if mfa_check is False:
                log.info('Resynchronize a multi-factor authentication TOTP token in case of time skew or failed checks.')
                ...
                log.info('Verify the token was successfully resynchronized by testing a new PIN')
                get_totp = reusable.generate_totp(SEED)
                cmd = f'time radosgw-admin mfa check --uid {user_id} --totp-serial {serial} --totp-pin {get_totp}'
        ...
        if config.test_ops['create_bucket'] is True:
            ...
            for bc in range(config.bucket_count):
                ...
                if config.test_ops.get('enable_mfa_version', False):
                    ...
                    token, status = reusable.enable_mfa_versioning(bucket, rgw_conn, SEED, serial, each_user, write_bucket_io_info)
                    if status is False:
                        ...
                        if token == new_token:
                            ...
                            time.sleep(30)
                        status = reusable.enable_mfa_versioning(bucket, rgw_conn, SEED, serial, each_user, write_bucket_io_info)
                        if status is False:
                            raise MFAVersionError('Failed to enable MFA and versioning on the bucket!')
                ...
                if config.test_ops['delete_mfa_object'] is True:
                    for s3_object_name, path in objects_created_list:
                        log.info('Deleting an object configured with MFA should have TOTP token')
                        ...
                        for version in versions:
                            ...
                            if object_delete is False:
                                raise TestExecError ("Object deletion with MFA token failed!")
            
                            log.info('Verify object is deleted permanently')
                            cmd = f'radosgw-admin bucket list --bucket {bucket_name_to_create}'
                            bucket_list=utils.exec_shell_cmd(cmd)
                            if version.version_id in bucket_list:
                                raise TestExecError ("Object version is still present, Failed to delete the object")
                ...
                if config.test_ops['remove_mfa'] is True:
                    log.info('Delete a multi-factor authentication (MFA) time-based one time password (TOTP) token')
                    ...
                    cmd = f'radosgw-admin mfa get --uid {user_id} --totp-serial {serial}'
