"""
test_swift_delete_during_sync.py - Test Swift all type of  objects expiration

Usage: test_swift_delete_during_sync.py -c <input_yaml>
<input_yaml>
    multisite_configs/test_swift_delete_during_sync.yaml
...
def test_exec(config, ssh_con):
    ...
    reusable.check_bucket_sync_status(container_name)
    sr.upload_dlo(rgw, container_name, TEST_DATA_PATH, filesize=20000)
    time.sleep(2)
    _, objects = rgw.get_container(container_name)
    log.info(f"OBJECTS BEFORE DELETION : {objects}")
    objects = sr.list_all_objects(rgw, container_name)
    for obj in objects:
        rgw.delete_object(container_name, obj.get("name"))
        log.info(f"Deleted : {obj.get('name')}")
    _, objects = rgw.get_container(container_name)
    log.info(f"OBJECTS AFTER DELETION : {objects}")
    reusable.check_bucket_sync_status(container_name)
