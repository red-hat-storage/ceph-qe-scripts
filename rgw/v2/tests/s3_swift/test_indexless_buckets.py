...
def test_exec(config) -> None:
    """
    This method executes the planned workflow.

    Args:
        config: Configuration to be use for testing.
    ...
    for each_user in all_users_info:
        ...
        # verify the buckets created has index_type = Indexless
        for bucket in buckets:
            log.info("verify the bucket created has index_type as Indexless")
            err_out, _ = utils.exec_shell_cmd(
                cmd=f"radosgw-admin bucket stats --bucket {bucket.name}",
                debug_info=True,
            )

            # BZ:2034637 - indicates unsupported operation
            if "indexless bucket" not in err_out:
                raise TestExecError(f"Unexpected outcome for {bucket}")

            # delete bucket and objects
            if config.test_ops["delete_bucket"]:
