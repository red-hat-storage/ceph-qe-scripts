                ...
                if config.test_ops["create_object"] is True:
                    ...
                    if config.test_ops.get("test_unicode_bi_list", False):
                        ...
                        if not config.test_ops.get("enable_version", False):
                            log.info("Enable bucket versioning for unicode bi list test")
                            reusable.enable_versioning(
                                bucket, rgw_conn, each_user, write_bucket_io_info
                            )
                        ...
                        op = utils.exec_shell_cmd(f"radosgw-admin bucket stats --bucket {bkt}")
                        json_doc = json.loads(op)
                        num_shards = json_doc["num_shards"]
                        log.info(f"Number of shards after resharding: {num_shards}")
                        if num_shards != 1:
                            raise TestExecError(f"Expected 1 shard but got {num_shards}")

                        # Create 300 objects with ASCII names (school1 to school300)
                        log.info("Creating 300 objects with ASCII names (school1 to school300)")
                        ...
                        log.info("Creating 300 objects with unicode names (école1 to école300)")
                        for i in range(1, 301):
                        ...
                        if duplicate_entries:
                            log.error(f"Found {len(duplicate_entries)} duplicate entries in bi list")
                            log.error(f"Duplicate entries: {duplicate_entries[:10]}")  # Show first 10
                            raise TestExecError(
                                f"bi list shows duplicate entries (backwards listing bug). "
                                f"Found {len(duplicate_entries)} duplicates"
                            )
                        else:
                            log.info("No duplicate entries found in bi list - test passed!")
