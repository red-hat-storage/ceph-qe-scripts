"""
test_acls_all_usrs.py - Test ACL operations with READ and WRITE permissions for all users

Usage: test_acls_all_usrs.py -c <input_yaml>

Operation:
    Test with read permission on buckets for all users
    Test with write permission on objects and buckets for all users
"""
...
def test_exec_read(config, ssh_con):
    ...
    try:
        ...
        for each_user in all_user_details_others:
            ...
            # Reset ACLs to private first (remove all grants except owner)
            for bucket_name in bucket_names:
                current_acl = rgw_conn1_c.get_bucket_acl(Bucket=bucket_name)
                grants_private = {
                    "Grants": [
                        {
                            "Grantee": {
                ...
                rgw_conn1_c.put_bucket_acl(
                    Bucket=bucket_name, AccessControlPolicy=grants_private
                )

            # Set READ permission for user2 on all buckets
            for bucket_name in bucket_names:
                current_acl = rgw_conn1_c.get_bucket_acl(Bucket=bucket_name)
                grants_read = {
                    "Grants": [
                ...
                rgw_conn1_c.put_bucket_acl(
                    Bucket=bucket_name, AccessControlPolicy=grants_read
                )
...
if __name__ == "__main__":
    test_info = AddTestInfo("test acls all usrs")
    test_info.started_info()
