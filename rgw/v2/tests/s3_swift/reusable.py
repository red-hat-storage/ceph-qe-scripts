...
def check_service_port(service, ssh_con=None):
    ...
    Args:
        service (str): Name of the service to check (e.g., 'rgw', 'mds', 'mon', 'httpd', 'nginx')
    ...
    try:
        ...
        if ssh_con is not None:
            orch_output = utremote_exec_shell_cmd(
...
def find_admin_socket(ssh=None):
    """
    ...
    socket_pattern = f"/var/run/ceph/{cluster_id}/ceph-client.rgw.*.asok"
    cmd = f"ls -t {socket_pattern} 2>/dev/null | head -1"

    if ssh:
        socket_path = ssh_con.exec_command(cmd).strip()
    else:
        socket_path = utils.exec_shell_cmd(cmd).strip()
    if not socket_path:
        raise Exception(f"RGW admin socket not found in {socket_path}")

    log.info(f"Found admin socket: {socket_path}")
    return socket_path


def get_perf_dump(ssh=None, return_json=True):
    """
    ...
    try:
        ...
        socket_path = find_admin_socket(ssh)
        cmd = f"ceph daemon {socket_path} perf dump"
