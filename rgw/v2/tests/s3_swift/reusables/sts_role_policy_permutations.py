...
# Prefix used by sts_permutations test objects (warehouse/test-object-...)
S3_PREFIX_FOR_CONDITION = "warehouse"

OBJECT_LEVEL_RESOURCE_TYPES = {
...
def prefixed_object_resources(tenant_name, bucket_name, prefix=S3_PREFIX_FOR_CONDITION):
    """Object ARNs under the prefix path used by permutation test objects."""
    return [
        f"arn:aws:s3::{tenant_name}:{bucket_name}/{prefix}",
        f"arn:aws:s3::{tenant_name}:{bucket_name}/{prefix}/",
        f"arn:aws:s3::{tenant_name}:{bucket_name}/{prefix}/*",
    ]


def bucket_resource_arn(tenant_name, bucket_name):
    return f"arn:aws:s3::{tenant_name}:{bucket_name}"


def _build_condition_block(case, role_userid=None):
...
def _action_uses_prefixed_object_resources(s3_action):
    """True when the IAM action operates on object keys under a prefix."""
    if s3_action == "s3:ListBucket":
        return False
    resource_types = sts_perm.s3_action_required_resource.get(s3_action, [])
    return any(rt in OBJECT_LEVEL_RESOURCE_TYPES for rt in resource_types)


def build_policy_statements(case, tenant_name, bucket_name, role_userid=None):
    """
    Build IAM policy statement(s) for a permutation case.

    For s3:prefix, ListBucket is evaluated against the bucket with a prefix
    condition; object-level actions use separate statements on the prefixed path.
    """
    if case.condition_name == "s3_prefix":
        prefix_condition = _build_condition_block(case, role_userid)
        list_statement = {
            "Effect": case.effect,
            "Action": ["s3:ListBucket"],
            "Resource": bucket_resource_arn(tenant_name, bucket_name),
            "Condition": prefix_condition,
        }
        if case.s3_action == "s3:ListBucket":
            return [list_statement]

        if _action_uses_prefixed_object_resources(case.s3_action):
            action_resources = prefixed_object_resources(tenant_name, bucket_name)
        else:
            action_resources = resolve_resources(
        ...
        return [list_statement, action_statement]

    return [
        build_policy_statement(case, tenant_name, bucket_name, role_userid)
    ]
...
def build_policy_statement(case, tenant_name, bucket_name, role_userid=None):
    ...
    condition_block = _build_condition_block(case, role_userid)
    if condition_block:
...
def run_single_permutation(
    ...
    try:
        ...
        role_statements = build_policy_statements(
            case, tenant_name, bucket_name, role_userid
        )
        role_policy = {"Version": "2012-10-17", "Statement": role_statements}
        log.info(
            "case %s: role_id=%s role_session_name=%s aws:userid=%s policy_statements=%s",
            case.case_id,
            role_id,
            role_session_name,
            role_userid,
            json.dumps(role_statements),
        )
