...
def create_bucket_skip_sync(
    bucket_name, rgw, user_info, endpoint=None, location=None, retries=3, wait_time=5
):
    """
    Create a bucket WITHOUT sync status checks (for full sync tests).

    In full sync tests, the secondary zone is stopped, so we cannot check sync status.
    This function creates a bucket on the primary zone without waiting for sync.

    This is a specialized version of reusable.create_bucket() that skips the multisite
    sync checks at the end, making it suitable for full sync testing where the secondary
    zone RGW services are intentionally stopped.

    Args:
        bucket_name: Name of the bucket to create
        rgw: RGW connection object
        user_info: User information dict with access_key
        endpoint: Optional endpoint URL to verify before bucket creation
        location: Optional location constraint for bucket
        retries: Number of endpoint verification retries (default: 3)
        wait_time: Wait time between endpoint verification retries (default: 5s)

    Returns:
        Bucket object if successful

    Raises:
        TestExecError: If bucket creation fails
    """
    ...
    log.info(f"Creating bucket (skip sync check): {bucket_name}")
    ...
    if created is not None:
        ...
        if response.status_code == 200:
            log.info(
                f"✓ Bucket created: {bucket_name} (sync check skipped for full sync test)"
            )
        else:
    ...
    # SKIP multisite sync checks - this is intentional for full sync tests
    # where secondary zone is stopped
    log.info("Skipping sync status check (full sync test mode)")
...
def verify_sync_using_bucket_stats(
    bucket_names, remote_ssh_con, max_retries=600, check_interval=30
):
    """
