    ...
    try:
        ...
        try:
            ...
            if 'ongoing-request="true"' in restore_status:
                log.warning(
                    f"Object in ongoing-request state, waiting {wait_time}s to verify..."
                )
                ...
                if 'ongoing-request="true"' in restore_status_2:
                    log.error(
                        "BUG DETECTED: Object stuck in ongoing-request=true state"
                    )
            ...
            log.info(
                "✓ Days=0 restore was handled correctly (not stuck in ongoing state)"
            )
            ...
            if error_code == "InvalidArgument" or "Days" in error_msg:
                ...
                log.warning(
                    f"Days=0 rejected with different error: {error_code} - {error_msg}"
                )
...
def verify_cloud_transition_bugs(
    ...
    if check_etag:
        try:
            check_etag_double_quotes_bug(s3_client, bucket_name, object_key, version_id)
