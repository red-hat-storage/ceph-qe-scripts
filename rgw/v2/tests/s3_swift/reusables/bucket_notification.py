...
def get_kafka_brokers_list():
    ...
    kafka_brokers_list = [
        ip_and_port.replace(":2181", "") for ip_and_port in ip_and_port_list
    ]
...
def create_topic_from_kafka_broker(topic_name, partitions=2, replication_factor=2):
    ...
    """
    log.info(f"create topic {topic_name} from kafka broker")
    cmd = f"/usr/local/kafka/bin/kafka-topics.sh --create --topic {topic_name} --bootstrap-server kafka://10.0.65.137:9092 --partitions {partitions} --replication-factor {replication_factor}"
    utils.exec_shell_cmd(cmd)
    cmd = f"/usr/local/kafka/bin/kafka-topics.sh --describe --topic {topic_name} --bootstrap-server kafka://10.0.65.137:9092"
    utils.exec_shell_cmd(cmd)
