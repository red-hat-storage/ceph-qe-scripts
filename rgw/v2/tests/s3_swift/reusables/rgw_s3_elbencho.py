...
def get_rgw_services_by_realm(ssh_con=None, site_name="local", realm_name=None):
    """
    Get RGW services filtered by their actual realm configuration.

    Uses:
    1. ceph config dump | grep <realm_name> - to find client sections with that realm
    2. ceph orch ls - to list all RGW services
    3. Matches services based on configured realm

    Args:
        ssh_con: SSH connection (None for local)
        site_name: Name of site for logging
        realm_name: Realm name to filter by (e.g., "usa")

    Returns:
        dict: {
            'services_to_process': [service_names],  # Services matching the realm
            'all_services': [service_names],         # All RGW services found
        }
    """
    log.info(
        f"Identifying RGW services by checking actual configuration on {site_name}..."
    )

    # Step 1: Get all RGW services from ceph orch ls
    # Use --service-type instead of grep to avoid pipe issues with Paramiko
    # Use 'sh -c' to ensure proper shell environment for remote commands
    list_cmd = "ceph orch ls --service-type rgw"
    log.info(f"  Running: {list_cmd}")

    if ssh_con:
        # Use get_pty=True for Ceph commands to work properly in SSH sessions
        stdin, stdout, stderr = ssh_con.exec_command(f"sudo {list_cmd}", get_pty=True)
    ...
    if not all_services:
        log.info(f"  No RGW services found on {site_name}")
        return {
            "services_to_process": [],
            "all_services": [],
        }

    log.info(f"  Found {len(all_services)} RGW service(s): {all_services}")

    # Step 2: If no realm filter, return all services
    if not realm_name:
        log.info(
            f"  No realm filter specified, will process all {len(all_services)} service(s)"
        )
        return {
    ...
    # Step 3: Find daemons configured with the specified realm using ceph config dump
    # Run full command and filter in Python to avoid pipe issues with Paramiko
    config_cmd = "ceph config dump"
    log.info(
        f"  Running: {config_cmd} (will filter for realm '{realm_name}' in Python)"
    )

    if ssh_con:
        # Use get_pty=True for Ceph commands to work properly in SSH sessions
        stdin, stdout, stderr = ssh_con.exec_command(f"sudo {config_cmd}", get_pty=True)
        full_config_output = stdout.read().decode()
    else:
        full_config_output = utils.exec_shell_cmd(f"sudo {config_cmd}")

    # Filter output for realm_name in Python
    config_output = "\n".join(
        [line for line in full_config_output.split("\n") if realm_name in line]
    )

    # Parse the config output to find client.rgw.* sections with rgw_realm = <realm_name>
    # Format: "client.rgw.io.81  advanced  rgw_realm  usa  *"
    # Columns: section, level, name, value, mask
    daemon_identifiers = set()

    log.info(f"  Parsing config dump for realm '{realm_name}':")
    for line in config_output.split("\n"):
        if line.strip() and "client.rgw." in line and "rgw_realm" in line:
            parts = line.split()
            if len(parts) >= 4:
                section = parts[0]  # e.g., "client.rgw.io.81"
                # Skip parts[1] which is the level (e.g., "advanced")
                param = parts[2]  # e.g., "rgw_realm"
                value = parts[3]  # e.g., "usa"

                if param == "rgw_realm" and value == realm_name:
                    # Extract the service identifier from section
                    # "client.rgw.io.81" -> "io.81"
                    # "client.rgw.io.81.cali002.xyz" -> "io.81"
                    if section.startswith("client.rgw."):
                        parts_after_rgw = section.replace("client.rgw.", "").split(".")
                        # Take first 2 parts as service identifier (e.g., "io.81", "sync.80")
                        if len(parts_after_rgw) >= 2:
                            service_id = f"{parts_after_rgw[0]}.{parts_after_rgw[1]}"
                            daemon_identifiers.add(service_id)
                            log.info(
                                f"    Found daemon: {service_id} (from section {section})"
                            )

    if not daemon_identifiers:
        log.info(f"  No daemons found configured with realm '{realm_name}'")
        return {
            "services_to_process": [],
            "all_services": all_services,
        }

    log.info(f"  Daemon identifiers for realm '{realm_name}': {daemon_identifiers}")

    # Step 4: Match service names from 'ceph orch ls' to the daemon identifiers
    services_to_process = []

    for service_name in all_services:
        # Service name format: "rgw.io.81", "rgw.sync.80", "rgw.shared.sec.io", "rgw.shared.sec.sync"
        # Extract identifier: "io.81", "sync.80", "shared.sec.io", "shared.sec.sync"
        if service_name.startswith("rgw."):
            service_id = service_name.replace("rgw.", "", 1)

            # Check if service_id starts with any daemon identifier
            # This handles cases where service_id is "shared.sec.io" and daemon_identifier is "shared.sec"
            matched = False
            for daemon_id in daemon_identifiers:
                if service_id.startswith(daemon_id):
                    matched = True
                    break

            if matched:
                services_to_process.append(service_name)
                log.info(f"  ✓ Service '{service_name}' matches realm '{realm_name}'")
            else:
                log.info(
                    f"  ✗ Service '{service_name}' does not match realm '{realm_name}'"
                )
    ...
    log.info(
        f"  Result: {len(services_to_process)} service(s) to process out of {len(all_services)} total"
    )
    return result
...
def get_remote_endpoint_elbencho(zone_name=None, ssh_con=None):
    ...
    try:
        ...
        stdin, stdout, stderr = ssh_con.exec_command(
            "sudo ceph orch host ls", get_pty=True
        )
...
def stop_rgw_sync_services(ssh_con=None, site_name="secondary"):
    ...
    try:
        ...
        if ssh_con:
            stdin, stdout, stderr = ssh_con.exec_command(
                "sudo ceph orch ls", get_pty=True
            )
            orch_ls_output = stdout.read().decode()
        else:
...
def stop_rgw_services(
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            # Use new function to identify RGW services by their actual realm configuration
            service_info = get_rgw_services_by_realm(
                ssh_con=ssh_con, site_name=site_name, realm_name=realm_name
            )

            rgw_services = service_info["services_to_process"]
            all_services = service_info["all_services"]
            ...
            if ssh_con:
                stdin, stdout, stderr = ssh_con.exec_command(
                    f"sudo {verify_cmd}", get_pty=True
                )
