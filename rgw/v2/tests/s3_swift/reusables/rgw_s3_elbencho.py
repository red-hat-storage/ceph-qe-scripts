...
def get_rgw_services_by_realm(ssh_con=None, site_name="local", realm_name=None):
    ...
    """
    log.info(f"Identifying RGW services by checking actual configuration on {site_name}...")
    ...
    if not realm_name:
        log.info(f"  No realm filter specified, will process all {len(all_services)} service(s)")
        return {
            'services_to_process': all_services,
            'all_services': all_services,
        }

    # Step 3: Find daemons configured with the specified realm using ceph config dump | grep <realm_name>
    config_cmd = f"ceph config dump | grep {realm_name}"
    log.info(f"  Running: {config_cmd}")

    if ssh_con:
        stdin, stdout, stderr = ssh_con.exec_command(f"sudo {config_cmd}")
        config_output = stdout.read().decode()
    else:
        config_output = utils.exec_shell_cmd(f"sudo {config_cmd}")
                ...
                if param == 'rgw_realm' and value == realm_name:
                    ...
                    if section.startswith('client.rgw.'):
                        ...
                        if len(parts_after_rgw) >= 2:
                            ...
                            log.info(f"    Found daemon: {service_id} (from section {section})")
    ...
    for service_name in all_services:
        ...
        if service_name.startswith('rgw.'):
            ...
            if matched:
                ...
                log.info(f"  ✗ Service '{service_name}' does not match realm '{realm_name}'")
    ...
    log.info(f"  Result: {len(services_to_process)} service(s) to process out of {len(all_services)} total")
    return result
