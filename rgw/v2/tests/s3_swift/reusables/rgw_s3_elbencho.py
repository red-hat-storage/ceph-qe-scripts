    ...
    for attempt in range(1, max_retries + 1):
        ...
        for bucket in buckets:
            ...
            if not local_stats or not remote_stats:
                log.error(f"Failed to fetch valid JSON stats for bucket: {bucket}")
                mismatched_buckets.append(bucket)
                continue

            # Compare num_objects and size_actual across sites
            keys_to_compare = ["num_objects", "size_actual"]
            for key in keys_to_compare:
                if local_stats.get(key) != remote_stats.get(key):
                    mismatched_buckets.append(bucket)
                    log.error(
                        f"Mismatch in bucket {bucket} for key '{key}': Local={local_stats.get(key)}, Remote={remote_stats.get(key)}"
                    )
                    break  # No need to check further, already a mismatch

        if not mismatched_buckets:
            elapsed_time = int(time.time() - start_time)
            log.info(f"Sync got consistent in {elapsed_time} seconds.")
            return

        log.info(
            f"Waiting for bucket sync... Retrying in {sleep_interval} seconds. Attempt {attempt}/{max_retries}. Mismatched: {mismatched_buckets}"
        )
        time.sleep(sleep_interval)

    log.error(
        f"[SYNC ISSUE] The following buckets did not sync within {max_retries * sleep_interval} seconds ({max_retries / 2} hours): {mismatched_buckets}"
    )
