...
class ErrorToRevert(object):
    ...
    def ecom1_err_revert(self):
        try:
            ...
            op=subprocess.call(['mv', '/etc/ceph/%s' % (bkp_conf_file_name), '/etc/ceph/%s' % (conf_file)])
    ...
    def wosd1_revert(self):
        try:
            osd_names = subprocess.Popen(("ceph osd tree | grep host | awk '{print $4}'"), shell=True,
            ...
            stdin, stdout, stderr = ssh.exec_command("cd /var/lib/ceph/osd/* ;sudo rm ceph_fsid1")
